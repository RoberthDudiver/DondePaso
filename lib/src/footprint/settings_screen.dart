...
class _OverviewScreenState extends State<_OverviewScreen> {
  ...
  String? _primaryZoneName;

  @override
  void initState() {
    super.initState();
    _loadPrimaryZoneName();
  }

  Future<void> _loadPrimaryZoneName() async {
    final zone = widget.zonesSnapshot.primaryZone;
    if (zone == null) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    final strings = context.strings;
    final primaryZone = widget.zonesSnapshot.primaryZone;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          _SectionPanel(
            title: widget.progression?.title ?? strings.level,
            subtitle: widget.progression == null
                ? strings.generalOverviewBody
                : strings.levelValue(widget.progression!.level),
            child: Wrap(
              spacing: 12,
              runSpacing: 12,
              children: [
                _MetricCard(
                  ...
                  value: strings.formatCompactNumber(widget.totalPoints),
                ),
                _MetricCard(
            ...
            child: Wrap(
              ...
              children: [
                _MetricCard(
                  ...
                  value: strings.formatCompactNumber(widget.dailySteps),
                ),
                _MetricCard(
                  label: strings.activeZones,
                  value: '${widget.zonesSnapshot.zones.length}',
                ),
                _MetricCard(
                  label: strings.unlockedAchievements,
                  value: widget.progression == null
                      ? '0'
                      : '${widget.progression!.unlockedAchievements}',
                ),
                _MetricCard(
                  label: strings.level,
                  value: widget.progression == null
                      ? '1'
                      : '${widget.progression!.level}',
                ),
              ],
            ),
          ),
          if (primaryZone != null) ...[
            ...
            _SectionPanel(
              ...
              child: _ZoneSummaryRow(
                zone: primaryZone,
                displayName: _primaryZoneName,
              ),
            ),
          ],
        ],
