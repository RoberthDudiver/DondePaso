...
class FootprintTimelapseService {
  ...
  static const int _introFrameCount = 18;
  static const int _holdFrameCount = 10;
  ...
  static Future<void> generateAndShare({
    ...
    final frameCount = math.min(28, math.max(16, (sortedCells.length * 0.55).ceil()));
  ...
  static Future<List<File>> _renderFrames({
    ...
    for (var introIndex = 0; introIndex < _introFrameCount; introIndex++) {
      final introRatio = introIndex / math.max(1, _introFrameCount - 1);
    ...
    for (var frameIndex = 0; frameIndex < frameCount; frameIndex++) {
      final revealRatio = _easeOut((frameIndex + 1) / frameCount);
    ...
    if (frameFiles.isNotEmpty) {
      ...
      for (var holdIndex = 0; holdIndex < _holdFrameCount; holdIndex++) {
        final holdFile = File(
  ...
  static Future<File> _encodeVideo({
    ...
    commandParts.addAll([
      ...
      '-vf',
      '"format=yuv420p,scale=720:1280:flags=lanczos,fps=14"',
      '-c:v',
      'mpeg4',
      '-q:v',
      '3',
  ...
  static Future<ui.Image> _renderIntroFrame({
    ...
    _paintBackdrop(canvas, rect, introRatio * 0.25);
    ...
    _paintText(
      canvas,
      strings.timelapseIntroBody(zoneName),
      const Offset(56, 600),
      24,
      FontWeight.w500,
  ...
  static Future<ui.Image> _renderFrame({
    ...
    _paintBackdrop(canvas, backgroundRect, revealRatio);
        ...
        Paint()
          ..shader = RadialGradient(
            colors: [
              ...
              const Color(0xFFFFB348).withValues(alpha: 0.08),
              const Color(0x00000000),
            ],
  ...
  static void _paintBackdrop(Canvas canvas, Rect rect, double revealRatio) {
    canvas.drawRect(
      rect,
      Paint()
        ..shader = const LinearGradient(
    ...
    canvas.drawOval(
      ...
      Paint()
        ..shader = RadialGradient(
          colors: [
            const Color(0xFFFFB145).withValues(alpha: 0.07 + revealRatio * 0.04),
            const Color(0x00000000),
          ],
  ...
  static void _paintExplorationMass(
    ...
    canvas.drawPath(
      unionPath,
      Paint()
        ..color = const Color(0xFFFFA11A).withValues(alpha: 0.11 + revealRatio * 0.07)
    ...
    for (var index = 0; index < cells.length; index++) {
      ...
      canvas.drawPath(
        ...
        Paint()
          ...
          ..color = fillColor.withValues(alpha: isRecent ? 0.88 : 0.78),
      );
      canvas.drawPath(
        entry.path,
        Paint()
          ...
          ..color = borderColor.withValues(alpha: isRecent ? 0.78 : 0.28),
      );
    }
